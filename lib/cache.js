...
module.exports = class Cache {
  ...
  async cacheReleaseList(assetUrl) {
    ...
    for (let i = 0; i < matches.length; i += 1) {
      ...
      const platformName = `exe${fileName.includes('arm64') ? '_arm64' : ''}`
      const downloadUrl = assetUrl.includes('RELEASES')
        ? assetUrl.replace('RELEASES', fileName) : `${url}/download/${platformName}/${fileName}`
  ...
  async refreshCache() {
    ...
    for (const asset of release.assets) {
      ...
      this.latest.platforms[platform] = {
        name,
        api_url: url,
        url: this.config.url + '/download/' + platform + '?update=true',
        content_type,
